...
OpenCode Go est conçu pour être utilisé avec [OpenCode](https://opencode.ai) et d'autres agents de codage populaires qui génèrent des types de requêtes similaires.
...
3\. inclut l'en-tête `x-opencode-session` afin que nous puissions optimiser la mise en cache des prompts
...
L'allocation effective varie selon le modèle ; consultez le tableau ci-dessous.
...
Avec Go, vous payez 10 $/mois et, pour la plupart des modèles, nous cherchons à vous offrir un volume d'utilisation équivalant à 6 fois ce montant.
